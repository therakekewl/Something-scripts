...
local function StartScript()
    ...
    local function AddESP(p)
        local Box = Drawing.new("Square"); Box.Visible = false; Box.Thickness = 2; Box.Color = Config.ESPColor
        local Health = Drawing.new("Line"); Health.Visible = false; Health.Thickness = 2; Health.Color = Color3.new(0,1,0)
        local Name = Drawing.new("Text"); Name.Visible = false; Name.Size = 16; Name.Center = true; Name.Outline = true; Name.Color = Config.ESPColor
        ...
        local connection; connection = RunService.RenderStepped:Connect(function()
                ...
                if Config.ESP then
                    ...
                    if onScreen then
                        ...
                        if Config.ESPHealth and p.Character:FindFirstChild("Humanoid") then
                            Health.Visible = true
                            Health.From = Vector2.new(topLeft.X - 5, topLeft.Y + sizeY)
                            Health.To = Vector2.new(topLeft.X - 5, topLeft.Y + sizeY - (sizeY * (p.Character.Humanoid.Health / 100)))
                        else Health.Visible = false end

                        if Config.ESPNames then
                            Name.Visible = true
                            Name.Position = Vector2.new(pos.X, topLeft.Y - 20)
            ...
            if not p.Parent then Box:Remove(); Health:Remove(); Name:Remove(); connection:Disconnect() end
        end)
    end
    ...
    RunService.RenderStepped:Connect(function()
        ...
        local screenCenter = Vector2.new(Mouse.X, Mouse.Y + guiInset.Y)
        
        FOV.Visible = Config.Aimlock
        FOV.Radius = Config.AimFOV
        ...
        if Config.Aimlock and (UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) or IsMobile) then
            ...
            for _, v in pairs(Players:GetPlayers()) do
                if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild(Config.AimPart) then
                    ...
                    if onS then
                        local mDist = (Vector2.new(sPos.X, sPos.Y) - screenCenter).Magnitude
                        if mDist < minD then
                            local vis = true
                            ...
                            if vis then minD = mDist; targetInFOV = part end
                        end
                    end
                end
            end
            ...
            if targetInFOV then 
                Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, targetInFOV.Position + (targetInFOV.Velocity * Config.Prediction)), Config.Smoothness) 
            end
        end
    end)
